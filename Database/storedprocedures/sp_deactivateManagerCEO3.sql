...
********************************************/
DROP PROCEDURE IF EXISTS sp_deactivateManagerCEO3;

DELIMITER $$
CREATE PROCEDURE sp_deactivateManagerCEO3(
		...
		THEN 
			...
			IF EXISTS(SELECT 1 FROM tbl_corporate_deactivate_requests where username = inusername)
				...
				THEN
					...
					update tbl_corporate_deactivate_requests set ceo3 = 1 where username = inusername;
