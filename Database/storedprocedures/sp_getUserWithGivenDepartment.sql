...
BEGIN
	 ...
	 IF EXISTS(SELECT 1 FROM tbl_all_users WHERE username = inusername and department = department)
	 
		THEN 
			
			SELECT username, firstname, lastname, emailid, department, ssn, createdby, createddate from tbl_all_users where username = inusername and department = department;
